...
#Load combined data
df = pd.read_csv('C:/Users/leonw/Documents/Studium/6.Semester/Data Literacy/Hausarbeit/bike data/Clustered data/k20_cluster.csv')
...
# cluster 0
filtered_values_0 = np.where((df['cluster']==12))
...
df_0 = df_0.groupby(["station_id",'hour']).agg({'demand':'mean'})

df_0.reset_index(inplace=True)

df_0_piv = df_0.pivot(index='station_id', columns='hour', values='demand')
...
df_0['cluster_dtw'] = df_0['cluster_dtw'] + 120
df_0.head()

f =sns.lineplot(x="hour", y="demand", hue="cluster_dtw",data=df_0, legend='full')
plt.show()

df_0.drop('hour', axis=1, inplace=True)
df_0.drop('demand', axis=1, inplace=True)
len(pd.unique(df_0['station_id']))
df_0.groupby(['station_id','cluster_dtw']).cluster_dtw.count()
df = df_0.drop_duplicates()

df.to_csv(r'C:\Users\leonw\Documents\Studium\6.Semester\Data Literacy\Hausarbeit\bike data\Clustered data\cluster_13_d20.csv', index = False)